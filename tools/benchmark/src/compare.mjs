...
function key(result) { return `${result.backend}:${result.case.id}`; }
...
for (const result of candidate) {
  ...
  if (!previous) { rows.push({ case: key(result), status: "new" }); continue; }
  const metrics = [
  ...
  if (previous.gpuMs?.p95 !== undefined && result.gpuMs?.p95 !== undefined) {
    metrics.push(["gpu.p95", previous.gpuMs.p95, result.gpuMs.p95]);
  }
