...
export class WebGL2DeformationRenderer {
  ...
  constructor(canvas: HTMLCanvasElement, pkg: LoadedA2DPackage) {
    ...
    pkg.model.parameters.forEach((p, i) => { defs[i*4]=p.min; defs[i*4+1]=p.max; defs[i*4+2]=p.default; });
    ...
    if (influenceRef) {
      ...
      for (let i = 0; i < influenceCount; i++) {
        const b = i * 16;
        influenceFloatData[i*4] = dv.getUint32(b, true);
    ...
    this.head = { pivot: data?.pivot ?? [0.5,0.5], radius: data?.radius ?? [0.5,0.6], depthScale: data?.depthScale ?? 0, perspective: data?.perspective ?? 0, yawGain: data?.yawGain ?? 1, pitchGain: data?.pitchGain ?? 1 };
  ...
  private createPart(pkg: LoadedA2DPackage, part: Part): PartGpu {
    ...
    if (part.mesh.proxyZ) {
      ...
      if (!(z instanceof Float32Array) || z.length !== vertexCount) throw new Error(`${part.id}: invalid proxyZ`);
      proxyZ = z;
    }
    ...
    if (part.mesh.influenceRanges) {
      ...
      if (!(r instanceof Uint32Array) || r.length !== vertexCount * 2) throw new Error(`${part.id}: invalid influenceRanges`);
      ranges = r;
    }
    ...
    const addFloat = (location: number, data: Float32Array, size: number) => {
      const b = gl.createBuffer(); if (!b) throw new Error("buffer allocation failed"); buffers.push(b); gl.bindBuffer(gl.ARRAY_BUFFER, b); gl.bufferData(gl.ARRAY_BUFFER, data, gl.STATIC_DRAW); gl.enableVertexAttribArray(location); gl.vertexAttribPointer(location, size, gl.FLOAT, false, 0, 0);
    };
    addFloat(0, positions, 2); addFloat(1, uvs, 2); addFloat(2, proxyZ, 1);
    const rb = gl.createBuffer(); if (!rb) throw new Error("buffer allocation failed"); buffers.push(rb); gl.bindBuffer(gl.ARRAY_BUFFER, rb); gl.bufferData(gl.ARRAY_BUFFER, ranges, gl.STATIC_DRAW); gl.enableVertexAttribArray(3); gl.vertexAttribIPointer(3, 2, gl.UNSIGNED_INT, 0, 0);
    const ib = gl.createBuffer(); if (!ib) throw new Error("buffer allocation failed"); buffers.push(ib); gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, ib); gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, indices, gl.STATIC_DRAW);
  ...
  private flushDirtyParameters(): void {
    ...
    if (!range) return;
    const { gl } = this;
    gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, this.parameterTexture);
    let index = range.start;
    while (index < range.endExclusive) {
      const x = index % this.parameterTexSize[0]; const y = Math.floor(index / this.parameterTexSize[0]);
  ...
  render(): number {
    const { gl } = this; const canvas = gl.canvas as HTMLCanvasElement; const dpr = globalThis.devicePixelRatio || 1;
    ...
    gl.viewport(0,0,canvas.width,canvas.height); gl.clearColor(0.06,0.06,0.07,0); gl.clear(gl.COLOR_BUFFER_BIT); gl.useProgram(this.program);
    ...
    gl.uniform2i(loc("u_parameterTexSize"), this.parameterTexSize[0], this.parameterTexSize[1]); gl.uniform2i(loc("u_influenceTexSize"), this.influenceTexSize[0], this.influenceTexSize[1]);
    ...
    gl.uniform2f(loc("u_headPivot"), this.head.pivot[0], this.head.pivot[1]); gl.uniform1f(loc("u_headDepthScale"), this.head.depthScale); gl.uniform1f(loc("u_headPerspective"), this.head.perspective); gl.uniform1f(loc("u_headYawGain"), this.head.yawGain); gl.uniform1f(loc("u_headPitchGain"), this.head.pitchGain);
    let drawCalls = 0;
    for (const p of this.parts) { gl.bindVertexArray(p.vao); gl.drawElements(gl.TRIANGLES, p.indexCount, p.indexType, 0); drawCalls++; }
    gl.bindVertexArray(null); return drawCalls;
  }

  destroy(): void {
    const { gl } = this;
    for (const p of this.parts) { gl.deleteVertexArray(p.vao); for (const b of p.buffers) gl.deleteBuffer(b); }
    gl.deleteTexture(this.parameterTexture); gl.deleteTexture(this.parameterDefTexture); gl.deleteTexture(this.influenceTexture); gl.deleteProgram(this.program);
  }
}
